// driver file for videos page
export const videoPages = [
	{
		name: 'cities',
		enLabel: 'City climates',
		enText: `What is special about city climates? Cities are living
					things, always growing and changing, needing to adapt to the
					needs of their populations and to an ever-changing climate,
					which may threaten the city. These three short clips cover
					the topics of urban climate, climate change resilience for
					city infrastructure and data use for mapping urban areas.
					Available in English and Chinese.`,
		cnLabel: '城市气候',
		cnText:
			'城市气候有什么特别之处?城市是不断在演变的，总是在扩张和变化。为了适应人口的需求和不断变化的气候，城市需要不断调整来应对这些威胁。这里的三个短片涵盖了城市气候、城市基础设施应对气候变化的能力和城市地区数据测绘使用等主题。三个视频均有中英文两个版本。',
		videos: {
			en: [
				{
					id: '',
					title: 'Quick guide to urban climate'
				},
				{
					id: '',
					title: 'Futureproofing cities'
				},
				{
					id: '',
					title: 'Mapping and modelling cities'
				}
			],
			cn: [
				{
					id: '',
					title: '城市气候的快速指南'
				},
				{
					id: '',
					title: '面向未来的城市'
				},
				{
					id: '',
					title: '绘制城市地图和城市建模'
				}
			]
		}
	},
	{
		name: 'findings',
		enLabel: 'Research findings',
		enText: `Hear directly from researchers in CSSP China on their
					collaborative UK-China work and the fascinating findings. Dr
					Liang Guo and Dr Nick Klingaman talk about tracing the
					sources of moisture that fall as rain and snow over China.
					Dr Buwen Dong talks about analysing heatwaves in China and
					his predictions for the second half of this century. Videos
					in English with Chinese subtitles available.`,
		cnLabel: '研究发现',
		cnText:
			'在这里你可以直接听取气候科学支持服务伙伴关系（CSSP）计划中国项目研究人员在合作过程中所展开的工作和有趣的发现。Liang Guo博士和Nick Klingaman博士谈到了他们对中国降雨和降雪中水分来源的追踪。Buwen Dong博士谈到了他对中国热浪的分析和对本世纪下半叶的预测。此视频为英文版本，并配有中文字幕。',
		videos: {
			en: [
				{
					id: '',
					title: 'The water accounting tool',
					cnTitle: '水分来源追踪工具'
				},
				{
					id: '0WGfTYf0',
					title:
						'The evidence on past, present and future heatwaves in China'
				}
			]
		}
	},
	{
		name: 'climate',
		disabled: true,
		enLabel: 'Infrastructure design',
		enText: `For those who want to explore their own hands-on
					information, CSSP China has produced practical tools for
					businesses, planners and decision-makers to use directly to
					find out more about the current and future climate. Here are
					three videos summarising three different tools which are
					tailored specifically for infrastructure and cities. Videos
					in English and with Chinese voiceover.`,
		cnLabel: '基础设施设计',
		cnText:
			'气候科学支持服务伙伴关系（CSSP）计划中国项目为那些想要亲自实践探索信息的人提供了实用的工具。此工具可供企业、规划师和决策者直接使用，帮助他们了解更多关于当前和未来的气候的信息。以下三个视频总结了三种为基础设施和城市量身定制的不同工具，并配有英文视频和中文配音。',
		videos: {
			en: [
				{
					id: '',
					title: 'City packs and urban heat service'
				},
				{
					id: '',
					title: 'Visualising the surface urban heat island'
				},
				{
					id: '',
					title: 'Climate risk assessments for infrastructure'
				}
			],
			cn: [
				{
					id: '',
					title: '城市包和城市高温服务'
				},
				{
					id: '',
					title: '界面可视化和城市热岛'
				},
				{
					id: '',
					title: '城市基础设施的气候风险评估'
				}
			]
		}
	},
	{
		name: 'renewables',
		disabled: true,
		enLabel: 'Renewable energy',
		enText: `Enabling renewable energy systems are at the heart of our
					net-zero future and the climate is a key part of efficient
					and beneficial solutions. Hear about three tools and
					services from scientists and engineers within the CSSP China
					project revealing information on the next season and the
					next decades to help planning and operation. Videos in
					English and with Chinese voiceover.`,
		cnLabel: '可再生能源',
		cnText:
			'实现可再生能源系统是达成二氧化碳的”净零排放“的核心，而气候是有效和有利的解决方案的关键部分。我们将从CSSP中国项目的科学家和工程师那里了解三种工具和服务，这些工具和服务提供了下一季和未来几十年的信息，以帮助规划和运行。网站中包含了有关的中英文视频。',
		videos: {
			en: [
				{
					id: '',
					title: `Climate Services for the European Energy Sector`
				},
				{
					id: '',
					title: `Seasonal climate prediction, with applications for 
								wind energy and for Yangtze rainfall`
				},
				{
					id: '',
					title: `A new energy analytics platform and its utility for 
								expanding the penetration of renewable energy`
				}
			],
			cn: [
				{
					id: '',
					title: `欧洲能源部门的气候服务`
				},
				{
					id: '',
					title: `季节性气候预测在风能和长江流域降水的应用`
				},
				{
					id: '',
					title: `新能源分析平台及其在扩大可再生能源渗透方面的效用`
				}
			]
		}
		/*},
	{
		name: 'audio',
		enLabel: 'Audio discussions',
		enText: `Coming soon`,
		cnLabel: '音频讨论',
		cnText: 'TODO: Chinese translation',
		audio: true,
		videos: {
			en: [
				{
					id: '',
					title: 'Audiocast'
				}
			]
		}*/
	}
]
